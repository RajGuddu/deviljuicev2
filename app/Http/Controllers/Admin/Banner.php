    ...
    public function add_edit_banner(Request $request, $id=null){
        ...
        if($request->isMethod('POST')){
            $rules = [
                'page' => 'required',
                // 'main_title' => 'required',
                'video'  => 'nullable|file|mimetypes:video/mp4,video/quicktime,video/x-msvideo,video/x-ms-wmv,image/gif|max:51200',
                'status' => 'required',
            ];
            ...
            if($validated){
                ...
                if ($request->hasFile('video')) {
                    if ($request->file('video')->isValid()) {
                        ...
                        do {
                            $videoFilename = 'banner-video-' . Str::random(8) . '.' . $extension;
                            $exists = $this->commonmodel->isExists('tbl_banner', ['video' => $videoFilename]);
                        } while ($exists);
                        ...
                        if ($path) {
                            ...
                            if (isset($_POST['video2']) && !empty($_POST['video2'])) {
                                Storage::disk('public_root')->delete('videos/' . $_POST['video2']);
                            }
