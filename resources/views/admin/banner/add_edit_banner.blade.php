				...
				<div class="app-card app-card-settings shadow-sm p-4">
					...
					<div class="app-card-body">
						<form class="" action="{{ url()->current() }}" method="post" enctype="multipart/form-data">
							...
							<div class="mb-3">
								<label for="video" class="form-label">Video (mp4, mov, avi, wmv, gif only)</label>
								<input type="file" class="form-control" id="video" name="video">
								<input type="hidden" name="video2" value="{{ $solo->video ?? '' }}">
							...
							<div class="mb-3">
								...
								<input type="hidden" name="image2" value="{{ $solo->image ?? '' }}">
							</div>
				...
				<div class="card ">
					<div class="card-body">
						<div class="d-flex align-content-start flex-wrap my-2">
							...
							@endif
							@if(isset($solo) && $solo->video != '')
								@php $isVideo = 1; @endphp
								<div class="img-box">
									<span class="cancel-icon"
										onclick="cancel_image_('tbl_banner','video','id', {{ $solo->id }},'v')">
										<i class="fa-solid fa-xmark" title="Cancel"></i>
									</span>

									<video width="100%" height="auto" controls>
										<source src="{{ url(VIDEO_PATH.$solo->video) }}" type="video/mp4">
										Your browser does not support the video tag.
									</video>

									<small class="image-title">Banner Video</small>
								</div>
							@endif
